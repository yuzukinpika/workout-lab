...
import {cleanSets,entryVolume,unitLabel,setsLabelText,setDisp,loadModeFromWeight,loadModeLabel,nextLoadMode,lastBodyweightMode} from './calc.js';
...
export function renderSelEx(){
  ...
  if(!setsState.length)setsState=[ghostSet(0,initialMode)];
  else{
    const blankInitial=setsState.length===1&&setsState[0].w===''&&setsState[0].reps===''&&(setsState[0].rpe===''||setsState[0].rpe==null);
    // 種目を切り替えたらゴーストは新しい種目の前回値で引き直す（旧種目の値を残さない）
    if(blankInitial)setsState=[ghostSet(0,initialMode)];
    else setsState.forEach((s,i)=>{const g=ghostSet(i,initialMode,setsState[i-1]);s.gw=g.gw;s.greps=g.greps;if(!['bw','plus','minus'].includes(s.m))s.m=g.m;});
  }
  renderSets();
}
...
function prevSets(){
  ...
  DB.entries.forEach((e,i)=>{if(e.exName===selEx.name&&(!latest||e.date>latest.date||(e.date===latest.date&&i>latestIndex))){latest=e;latestIndex=i;}});
  return latest?latest.sets:[];
}
function ghostSet(i,initialMode,prev=setsState[i-1]){
  const p=prevSets()[i],greps=p?String(p.reps??''):prev?String(prev.reps!==''?prev.reps:prev.greps||''):'',gw=p?String(Math.abs(p.w)||''):prev?String(prev.w!==''?prev.w:prev.gw||''):'';
  const m=selEx.equip==='bodyweight'?(p?loadModeFromWeight(p.w):prev&&LOAD_MODES.includes(prev.m)?prev.m:initialMode):'bw';
  return {w:'',reps:'',rpe:'',m,gw,greps};
}
export function renderSets(){
  ...
  $('#sets').innerHTML=setsState.map((s,i)=>`
    <div class="setrow"><span class="n">${i+1}</span>
      ...
      ${eq==='bodyweight'&&s.m==='bw'?'':`<div class="field"><small>${unitLabel(eq,s.m)}</small><input type="number" inputmode="decimal" placeholder="${s.w===''&&s.gw!==''?esc(s.gw):'kg'}" value="${esc(s.w)}" onchange="setVal(${i},'w',this.value)"></div>`}
      <div class="field"><small>回数</small><input type="number" inputmode="numeric" placeholder="${s.reps===''&&s.greps!==''?esc(s.greps):'回'}" value="${esc(s.reps)}" onchange="setVal(${i},'reps',this.value)"></div>
...
function materializeGhosts(){setsState.forEach(s=>{if(s.w===''&&s.gw!=='')s.w=s.gw;if(s.reps===''&&s.greps!=='')s.reps=s.greps;});renderSets();}
export function addSet(){materializeGhosts();const l=setsState[setsState.length-1],initialMode=selEx.equip==='bodyweight'?lastBodyweightMode(selEx.name):'bw';setsState.push(ghostSet(setsState.length,initialMode,l));renderSets();}
export function rmSet(i){setsState.splice(i,1);if(!setsState.length)setsState=[ghostSet(0,selEx&&selEx.equip==='bodyweight'?lastBodyweightMode(selEx.name):'bw')];renderSets();}
